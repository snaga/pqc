...
/*
 *
 * $Header: /cvsroot/pgpool/pgpool/pool_hba.c,v 1.1 2007/02/01 15:31:59 yamaguti Exp $
 *
...
#define PGPOOL_PAM_SERVICE "pqcd" /* Service name passed to PAM */
...
void ClientAuthentication(POOL_CONNECTION *frontend)
{
	...
	if (! hba_getauthmethod(frontend))
	{
		...
		pool_send_error_message(frontend, frontend->protoVersion, "XX000",
								...
								"See pqcd log for details.", __FILE__, __LINE__);
...
static char *recv_password_packet(POOL_CONNECTION *frontend)
{
	...
	pool_debug("received password packet from frontend for pqcd's HBA");
...
static void auth_failed(POOL_CONNECTION *frontend)
{
	...
	switch (frontend->auth_method)
	{
		case uaReject:
			snprintf(errmessage, messagelen,
					 "authentication with pqcd failed for user \"%s\": host rejected",
					 frontend->username);
			/*
...
/* 			break; */
		case uaTrust:
			snprintf(errmessage, messagelen,
					 "\"trust\" authentication with pqcd failed for user \"%s\"",
					 frontend->username);
			break;
/* 		case uaIdent: */
/* 			snprintf(errmessage, messagelen, */
...
/* 			break; */
		...
		case uaPAM:
			snprintf(errmessage, messagelen,
					 "PAM authentication with pqcd failed for user \"%s\"",
					 frontend->username);
			break;
#endif /* USE_PAM */
		default:
			snprintf(errmessage, messagelen,
					 "authentication with pqcd failed for user \"%s\": invalid authentication method",
					 frontend->username);
			break;
	}
...
static bool check_user(char *user, char *param_str)
{
		 ...
		 tok != NULL; tok = strtok(NULL, MULTI_VALUE_SEP))
	{
		if (tok[0] == '+')
		{
			 ...
			 * original code.
			 */
			pool_error("group token \"+\" is not supported in pqcd");
			return false;
...
static bool check_db(char *dbname, char *user, char *param_str)
{
		 ...
		 tok != NULL; tok = strtok(NULL, MULTI_VALUE_SEP))
	{
		...
		else if (strcmp(tok, "samegroup\n") == 0)
		{
			 ...
			 * original code.
			 */
			pool_error("group token \"samegroup\" is not supported in pqcd");
			return false;
...
static POOL_STATUS CheckPAMAuth(POOL_CONNECTION *frontend, char *user, char *password)
{
	...
	if (frontend->auth_arg && frontend->auth_arg[0] != '\0')
		retval = pam_start(frontend->auth_arg, "pqcd@",
						   &pam_passw_conv, &pamh);
	else
		retval = pam_start(PGPOOL_PAM_SERVICE, "pqcd@",
						   &pam_passw_conv, &pamh);
